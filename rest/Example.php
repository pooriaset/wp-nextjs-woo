...
class Example extends Base {
	...
	public function add_custom_field() {
		\register_rest_field(
			...
			array(
				...
				'schema'          => array(
					'description' => \__( 'Text field demo of Post type', S_TEXTDOMAIN ),
					'type'        => 'string',
				),
			)
		);
	...
	public function get_text_field( array $post_obj ) {
		...
		return \strval( \get_post_meta( $post_id, S_TEXTDOMAIN . '_text', true ) );
	}
	...
	public function update_text_field( string $value, \WP_Post $post, string $key ) {
		...
		if ( false === $post_id ) {
			return new \WP_Error(
				...
				\__( 'Failed to update post views.', S_TEXTDOMAIN ),
				array( 'status' => 500 )
			);
		}
