...
class AdminEnqueueCest {
	...
	function enqueue_admin_scripts(AcceptanceTester $I) {
		...
		$I->seeInPageSource('shop-core-plugin-settings-script');
		$I->seeInPageSource('shop-core-plugin-admin-script');
	}

	function enqueue_admin_styles(AcceptanceTester $I) {
		$I->wantTo('access to the plugin settings page and check the style enqueue');
		$I->amOnPage('/wp-admin/admin.php?page=shop-core-plugin');
		$I->seeInPageSource('shop-core-plugin-settings-styles-css');
		$I->seeInPageSource('shop-core-plugin-admin-styles-css');
	}
