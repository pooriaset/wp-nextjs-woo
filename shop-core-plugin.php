 ...
 * @link      
 *
 * Plugin Name:     shop-core-plugin
 * Plugin URI:      @TODO
 * Description:     @TODO
 * Version:         1.0.0
 * Author:          Pooria Setayesh
 * Author URI:      
 * Text Domain:     shop-core-plugin
 * License:         GPL 2.0+
...
define( 'S_TEXTDOMAIN', 'shop-core-plugin' );
define( 'S_NAME', 'shop-core-plugin' );
	...
	add_action(
		...
		static function() {
			...
			sprintf(
				...
				__( '"shop-core-plugin" requires PHP 5.6 or newer.', S_TEXTDOMAIN )
			)
			);
		}
	);
...
$shop_core_plugin_libraries = require S_PLUGIN_ROOT . 'vendor/autoload.php'; //phpcs:ignore
...
if ( ! wp_installing() ) {
	register_activation_hook( S_TEXTDOMAIN . '/' . S_TEXTDOMAIN . '.php', array( new \shop_core_plugin\Backend\ActDeact, 'activate' ) );
	register_deactivation_hook( S_TEXTDOMAIN . '/' . S_TEXTDOMAIN . '.php', array( new \shop_core_plugin\Backend\ActDeact, 'deactivate' ) );
	add_action(
		'plugins_loaded',
		static function () use ( $shop_core_plugin_libraries ) {
			new \shop_core_plugin\Engine\Initialize( $shop_core_plugin_libraries );
		}
	);
}
